...
def split_tiles(raster_data,tile_row,tile_col,params):
...
def if_split(raster_data,split_flag = 'auto',tile_row = None,tile_col = None):
    if (split_flag == 'auto'):
        ...
        if dataset_size > 3 * total_memory:
            ...
            tile_datasets = split_tiles(raster_data,tile_row,tile_col)
...
def get_traj(change_block,params = None):
    nodata_val = params['nodata_val']
    nt = params['nt']
...
def process_data(datasets,
    ...
    if (tiled is True):
        ...
        for ti in tqdm(range(len(datasets)), desc="Processing Tiles"):
            ...
            traj_outputs = compute_traj(input_xr,
                                        params,
                                        areaunit = areaunit, 
                                        weight = weight,
        ...
        traj_outputs = compute_traj(datasets,
                                    params,
                                    areaunit = areaunit, 
                                    weight = weight,
                                    chunk_size = chunk_size,
        ...
        if(save_tif is True):
            merged_traj.rio.to_raster(write_dir + 'traj_result.tif')
        print("Finished computing stats")
...
def compute_traj(input_xr,params,areaunit = 'perc_region', weight = False,chunk_size = 1000,tiled = False):
    ...
    reclassed_ = reclass(input_da,pres_val,nodata_val,chunking = chunking)   
    ...
    traj = da.map_blocks(lambda block: get_traj(block, params),ref_change,dtype = "int8",drop_axis = 0)
    xr_traj.data = traj
...
def plot_traj_stack(traj_result, areaunit, params,rotation = 90, ylim = None):
...
def get_components(traj_results,params,type_ = 'raster',comp_unit = 'perc_region'):
    global netstatus
    res = params['res']
    years = params['years']
